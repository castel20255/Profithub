...
export class DerivConnector extends EventEmitter {
    ...
    private async authorize(): Promise<void> {
        ...
        try {
            ...
            if (this.tokenRefreshAttempts < this.maxTokenRefreshAttempts) {
                console.log(
                    `[v0] Retrying authorization (${this.tokenRefreshAttempts}/${this.maxTokenRefreshAttempts})...`
                );
    ...
    private startHealthCheck(): void {
        ...
        this.healthCheckInterval = setInterval(() => {
            ...
            const now = Date.now();
            const timeSinceLastMessage = this.connectionState.lastMessageTime
                ? now - this.connectionState.lastMessageTime
                : 0;
