...
import { DerivWebSocket, type DerivSymbol, type ConnectionLog } from '@/profit-hub/lib/deriv-websocket';
    ...
    useEffect(() => {
        ...
        const unsubscribeTick = wsRef.current.subscribe('tick', data => {
            if (data.tick) {
                ...
                setRecentDigits(engineRef.current?.getRecentDigits(100) || []);
