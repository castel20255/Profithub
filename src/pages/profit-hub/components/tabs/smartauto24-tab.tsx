...
export function SmartAuto24Tab({ theme = 'dark' }: SmartAuto24TabProps) {
    ...
    const completeAnalysis = async () => {
        ...
        addAnalysisLog(
            `${selectedStrategy} Power: ${analysis.power.toFixed(1)}% - Signal: ${analysis.signal}`,
            'success'
        );
        ...
        if (selectedStrategy === 'Differs' && differsSelectedDigit !== null) {
            ...
            setDiffersTicksSinceAppearance(0);
            addAnalysisLog(
                `Waiting for digit ${differsSelectedDigit} to appear, then watching next 3 ticks...`,
                'info'
            );
        ...
        while (currentProfit < targetProfitAmount && Math.abs(currentProfit) < stopLossAmount) {
            ...
            try {
                ...
                setStats(prev => {
                    ...
                    newStats.winRate =
                        newStats.numberOfRuns > 0 ? (newStats.totalWins / newStats.numberOfRuns) * 100 : 0;
        ...
        >
            <div className='flex items-center justify-between mb-4 pb-3 border-b border-blue-500/20'>
                ...
                <div className='flex items-center gap-3'>
                    <h2
                        className={`text-base sm:text-lg font-bold ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}
                    >
                            ...
                            >
                                <div className='flex items-center justify-between'>
                                    ...
                                    <div className='text-right'>
                                        ...
                                        <div
                                            className={`text-sm font-bold ${theme === 'dark' ? 'text-orange-400' : 'text-orange-600'}`}
                                        >
                                            {market}
                                        </div>
                                    </div>
                        ...
                        >
                            ...
                            <div className='mb-8'>
                                <div className='flex justify-between items-center mb-3'>
                                    ...
                                    <span
                                        className={`text-sm font-bold ${theme === 'dark' ? 'text-yellow-400' : 'text-yellow-600'}`}
                                    >
                            ...
                            >
                                <h4
                                    className={`text-sm font-bold mb-3 ${theme === 'dark' ? 'text-gray-300' : 'text-gray-300'}`}
                                >
                                    Analysis Log
                                </h4>
